...
class UtilityToolkit:
    def __init__(self, root):
        self.root = root
        # 添加Tkinter高DPI支持和窗口设置
        self.setup_tkinter_environment()

        self.root.title("PiTools")
    ...
    def setup_tkinter_environment(self):
        """设置Tkinter环境，包括高DPI支持和窗口属性"""
        # 高DPI支持
        ...
        try:
            # 这里可以替换为你的图标路径
            if os.path.exists("pitools_icon.ico"):
                self.root.iconbitmap("pitools_icon.ico")
        except:
            pass  # 没有图标时忽略
    ...
    def setup_fonts_and_styles(self):
        ...
        self.style.configure("TLabelframe", font=bold_font)
    ...
    def change_theme(self, theme_name):
        ...
        try:
            ...
            self.status_bar.config(text=f"已切换到{theme_name}主题")
        except Exception as e:
            messagebox.showerror("错误", f"无法切换到{theme_name}主题: {str(e)}")
            self.status_bar.config(text=f"切换主题失败")
